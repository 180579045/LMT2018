...
namespace SCMTMainWindow.Component.SCMTControl
{
	...
	{
		//Timer tmr;
		private FlowChartCommand fcNodeCmd; // 流程图的对应的命令类

		private string fileXmlPath = FilePathHelper.GetAppPath() + ConfigFileHelper.FlowChartXml;
		...
		/// 初始化 : 获取每个流程图的对应的命令
		/// </summary>
		private void initflowChartCmdGetFlowChartCommand()
		{
			// dataHandle.resultInitData = new ResultInitData(ResultInitData);
			fcNodeCmd = new FlowChartCommand(fileXmlPath) { setColor = flowChartCmdModCanvasEllipse };
			// 设置流程图颜色
		}
		...
		/// 设置颜色
		/// </summary>
		private void flowChartCmdSetFlowChartColor()
		{
		...
		private void flowChartCmdModCanvasEllipse(Dictionary<string, string> modDic)
		{
		...
		private void initInterface()
		{
			...
			var document = XDocument.Load(fileXmlPath);
			var root = document.Root;
			var firttnode1 = (XElement)root.FirstNode;
			var firstnode2 = (XElement)firttnode1.FirstNode;
			var bb = firstnode2.Attribute("Stretch").Value;

			var enumerable = firstnode2.Elements();
			foreach (var item in enumerable)
			{
				var dd1 = item.Name.ToString();

				switch (dd1)
				{
					case "Canvas":
						//处理Canvas
						var dd = item.Attribute("Name").Value;
		...
		private void keepCanvasElement(string name, XElement element)
		{
			...
			{
				var aa = item.Name.ToString();
				switch (aa)
				{
					case "Ellipse":
						var bb = item.Attribute("Name").Value;
		...
		{
			var leafName = element.Attribute("leafName").Value;
		}
		...
		{
			var lineName = element.Attribute("Name").Value;
		...
		{
			var strName = element.Attribute("Name").Value;
		...
		private void paintCanvasEllipse(Canvas basicCanv)
		{
			foreach (var item in mapCanvasEllipse)
			{
				...
				var lefttoEllipse = element.Attribute("Canvas.Left").Value;
				var toptoEllipse = element.Attribute("Canvas.Top").Value;
				var widthEllipse = element.Attribute("Width").Value;
				var hightEllipse = element.Attribute("Height").Value;
				var nameEllipse = element.Attribute("Name").Value;
				var colorFillEllipse = element.Attribute("Fill").Value;
				var colorStrokeEllipse = element.Attribute("Stroke").Value;
				var matrixEllipse = "";
				foreach (var item1 in element.Elements())
				{
		...
		private void paintCanvasTextBlock(Canvas basicCanv)
		{
			foreach (var item in mapCanvasTextBlock)
			{
				...
				var stringText = element.Value;
				var textFontSize = element.Attribute("FontSize").Value;
				var textFontFamily = element.Attribute("FontFamily").Value;
				var textForeground = element.Attribute("Foreground").Value;
				var textCanvasLeft = element.Attribute("Canvas.Left").Value;
				var textCanvasTop = element.Attribute("Canvas.Top").Value;
				var textName = element.Attribute("Name").Value;
		...
		private void paintLine(Canvas basicCanv)
		{
			foreach (var item in mapLine)
			{
				...
				var lineX1 = element.Attribute("X1").Value;
				var lineY1 = element.Attribute("Y1").Value;
				var lineX2 = element.Attribute("X2").Value;
				var lineY2 = element.Attribute("Y2").Value;
				var lineName = element.Attribute("Name").Value;
				var lineFill = element.Attribute("Fill").Value;
				var lineStroke = element.Attribute("Stroke").Value;
		...
		private void paintExampleRectangle(Canvas basicCanv)
		{
			foreach (var item in mapExampleRectangle)
			{
				...
				var strCanvasLeft = element.Attribute("Canvas.Left").Value;
				var strCanvasTop = element.Attribute("Canvas.Top").Value;
				var strWidth = element.Attribute("Width").Value;
				var strHeight = element.Attribute("Height").Value;
				var strName = element.Attribute("Name").Value;
				var strFill = element.Attribute("Fill").Value;
				var strStroke = element.Attribute("Stroke").Value;
		...
		private void paintExampleText(Canvas basicCanv)
		{
			foreach (var item in mapExampleText)
			{
				...
				var strFontSize = element.Attribute("FontSize").Value;
				var strFontFamily = element.Attribute("FontFamily").Value;
				var strForeground = element.Attribute("Foreground").Value;
				var strCanvasLeft = element.Attribute("Canvas.Left").Value;
				var strCanvasTop = element.Attribute("Canvas.Top").Value;
				var strName = element.Attribute("Name").Value;
				...
				var canv = new Canvas();
				var myExampleText = new TextBlock();
	...
	internal class FlowChartCommand
	{
		/// <summary>
		/// [内存]每一个流程图中命令内容
		...
		public FlowChartCommand(string xmlFilePath)
		{
			...
			var nodeList = xmlDoc.SelectSingleNode("UserControl/Grid/Viewbox")?.ChildNodes;
			if (null == nodeList)
			{
				return;
			}
			...
			foreach (XmlNode xn in nodeList)
			{
				...
				var cmd = xn.SelectSingleNode("relatedcmd");
				// 3.2.2. 解析每个流程图的信息
				var flowChartName = xn.Attributes["Name"].Value; // 用于操作图形的句柄
		...
		private void fccFromXmlParseFlowChartOfCmdsInfo(string flowChartName, XmlNode cmds)
		{
			...
			if (null != cmds)
			{
				...
				{
					var leafList = fccFromCommandsParseCmdOfLeafsList(cmd);
					///
					var cmdName = cmd.Attributes["name"].Value;
		...
		{
			var cmdList = new Dictionary<string, List<Dictionary<string, string>>>();
			///解析每个命令
			foreach (XmlNode cmd in cmds.ChildNodes)
			{
				var leafList = new List<Dictionary<string, string>>();
				/// 解析每个节点
		...
		#endregion 0:解析xml信息.1.从xml解析每个流程图的cmdsInfo;->2.解析指定流程图的FlowChartOfCmdsInfo;->3.解析每个cmd的CmdOfLeafsInfo;->4.解析leafInfo属性.
		...
		{
			var whichWay = "wayOne";// "wayTwo"
			if (null == fcCmd)
				return null;

			var flowChartColor = new Dictionary<string, string>();
			...
			if (String.Equals(whichWay, "wayOne"))
			{
				...
				{
					var setColor = parseCmdInfo(flowChartName.ToString());
			...
			{
				/// 方案二 ： 同时点亮, 但没有返回值
				var threads = new Dictionary<Thread, object>();
				foreach (var flowChartName in fcCmd.Keys)
				{
					var ParStart = new ParameterizedThreadStart(parseCmdInfo);
		...
		{
			var flowChartName = ParObject.ToString();
			//Console.WriteLine("======== {0} ======start====", flowChartName);
			var leafList = fcCmd[flowChartName];
			if (null == leafList)
				return;
			var lastColor = DecisionAnalysisgetOneCmdColor("", leafList);
			var colr = new Dictionary<string, string>() { { flowChartName, lastColor } };
			setColor(colr);
		...
		{
			var color = "";
			var leafValueResult = new List<bool>();

			foreach (var leafDict in leafDictList)
			{
				var isLastLeaf = compLeafValueWithXmlValue(cmdName, leafDict);
				leafValueResult.Add(isLastLeaf);

				/// 逻辑 : 只要有一个叶子节点符合 跳过条件，就不继续判断
				var leafProperty = leafDict["leafProperty"];
		...
		{
			var oidPrefix = SnmpToDatabase.GetMibPrefix();                        // oid 前缀
			var boardAddr = CSEnbHelper.GetCurEnbAddr();                              // 板卡地址
			if (null == boardAddr)
			{
				throw new CustomException("尚未选中基站");
			}
			var lmtbSnmpEx = DTLinkPathMgr.GetSnmpInstance(boardAddr);// snmp 操作句柄
			var lmtVbs = new List<CDTLmtbVb>() {                 // snmp 查询的输入oid的容器
				new CDTLmtbVb() { Oid = (oidPrefix + leafOid) }};            //("1.3.6.1.4.1.5105.100.2.4.1.1.2.1.1.11");
			var resultsList = new List<string>();                   // 所有的有效值
			Dictionary<string, string> tmpResult;                            // 单次查询结果
			var logMsg = new List<string>();                       // 定位记录
																   // 遍历查询所有有效的数据
			while (true)
			{
				if (lmtbSnmpEx.GetNextRequest(boardAddr, lmtVbs, out tmpResult, 0))
				{
					...
					foreach (var item in tmpResult)
					{
						...
						resultsList.Add(item.Value);                       // 保存结果
																		   // 为下次snmp取值做准备
						var lmtVbTmp = new CDTLmtbVb { Oid = (item.Key) };
						lmtVbs.Add(lmtVbTmp);
					}
				}
				else
		...
		/// </summary>
		private void GetAllSnmpDataForEachLeafNode()
		{
			var boardAddr = CSEnbHelper.GetCurEnbAddr();                              // 板卡地址
			if (null == boardAddr)
			{
			...
			var lmtbSnmpEx = DTLinkPathMgr.GetSnmpInstance(boardAddr);     /// 查询snmp的句柄
			var lmtVbs = new List<CDTLmtbVb>();                       /// 多个叶子节点组成的队列
																	  /// 一起查询多个叶子节点内容
			...
			var result = new Dictionary<string, string>(); /// snmp返回的结果汇总
		...
		private bool compLeafValueWithXmlValue(string cmdName, Dictionary<string, string> leafDict)
		{
			...
			// 获取所有有效的(行状态有效)数值
			var effectiveVal = GetsAllSnmpValuesForALeafNode(mapNameData[mibName].childOid);
			/// 逻辑 : 只要有一个索引的值符合条件，就可以
			foreach (var value in effectiveVal)
			{
				Console.WriteLine("====** mibname=({0}) value = ({1})", mibName, value);
				var compResult = DecisionAnalysisgetFlowChartColor(leafDict, value);
				if (compResult)
				{
					isLastLeaf = true;
					break;
		...
		{
			var indexMax = int.Parse(leafDict["indexNum"]);
			if (indexNum == 0 || indexNum > indexMax)
			{
				return;
			}

			var indexNo = "index" + indexNum.ToString();
			var index = splitIndexValueForStartEnd(indexNo, leafDict[indexNo]);
			for (var start = index["start"]; start <= index["end"]; start++)
			{
				var postfixIndex = prefixIndex + "." + start.ToString();

				var indexNext = indexNum + 1;
		...
		{
			var indexOf = indexValue.IndexOf('.');
			if (-1 == indexOf)
			{
				// meiyou
				return new Dictionary<string, int>() { { "start", 0 }, { "end", 0 } }; ;
			}
			var intStart = int.Parse(indexValue.Substring(0, indexOf));
			var intEnd = int.Parse(indexValue.Substring(indexOf + 2));
			return new Dictionary<string, int>() { { "start", intStart }, { "end", intEnd } };
		}

		#endregion [废弃]原因 : 使用 snmp 的 getNext 功能 : 可以连续只获取有效行状态的实例的节点值，不用进行无效循环.
		...
		private bool DecisionAnalysisgetFlowChartColor(Dictionary<string, string> leafInfo, string snmpReturnLeafValue)
		{
			...
			var compRules = new Dictionary<string, CompDataResult>() {
			...
			var mibComp = leafInfo["leafCompRules"];
			var compResult = compRules[mibComp](snmpLeafVal, xmlValue);
