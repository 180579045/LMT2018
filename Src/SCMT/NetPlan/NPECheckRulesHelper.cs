...
namespace NetPlan
{
    ...
    public class NPECheckRulesHelper
    {
        //前台UI初始当前基站mib数据,
        ...
        public EnumResultType GetPropertyConditionValue(string property, DevAttributeInfo curRecord)
        {
            ...
            //计算最终的结果，条件表达式进行运算
            string result;
            if (!CommCheckRuleHelper.CalculateConditionExpr(property, propertyValueDic, out result))
            {
        ...
        public bool GetQueryFatherName(string exptr, Dictionary<string, object> queryDic, out string fatherName)
        {
            ...
            foreach (var oneQuery in queryDic)
            {
                ...
                int index = CommCheckRuleHelper.GetIndex(exptr, ".", out length);
                query1 = (index == -1) ? (exptr + ".") : exptr.Substring(0, index + length);

                index = CommCheckRuleHelper.GetIndex(oneQuery.Key, query1, out length);
                if (-1 != index)
                {
        ...
        public EnumResultType TakeRoundRulesConditionalResult(string rules, DevAttributeInfo curRecord, Dictionary<string, Object> queryDic, out bool finalResult)
        {
            ...
            //1.获取条件语句使用到的参数名
            if (!CommCheckRuleHelper.GetParaByConditionalExp(rules, out conditionalNameList))
            {
                //获取条件参数失败，直接跳过该条规则
        ...
        public bool IsMibTableOfQueryName(string queryName, Dictionary<string, object> queryDic, out string realQueryName)
        {
            ...
            int length;
            int index = CommCheckRuleHelper.GetIndex(queryFatherName, ".", out length);
            if (-1 == index)
            {
            ...
            Dictionary<string, object> queryDic)
        {
            ...
            if (false == CommCheckRuleHelper.SplitQueryExpr(rules, out splitDic))
            {
            ...
            if (!CommCheckRuleHelper.GetParaByQueryExpr(splitDic, out paraDic))
            {
                return EnumResultType.fail;
            }
            ...
            foreach (var tmp in fromParaValueDic)
            {
                ...
                string strWhere = CommCheckRuleHelper.GetFilterWhere(realWhere, whereParaValueDic, out realWhereParaList);
                try
                {
        ...
        public EnumResultType GetRoundCheckValue(List<RoundRule> exp, DevAttributeInfo curRecord)
        {
            ...
            {
                loop++;
                EXPRESSIONTYPE type = CommCheckRuleHelper.ExpressionIsConditionOrQuery(round.rules);
                //约定最后一个表达式必须为条件语句
