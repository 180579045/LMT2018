...
namespace CfgFileOperation
{
    ...
    class TestCfgForInitAndPatch
    {
        ...
        public void Main()
        {
            ...
            //new TestCfgForInitAndPatch().BeyondComparePatchExCfgMain();
            Console.WriteLine("BeyondComparePatchExCfgMain");
            BeyondComparePatchExCfgMain(args);
        ...
        public void BeyondComparePatchExCfgMain(string[] args)
        {
            ...
            string writelog = dataBasePath+"5GCfg\\80patch20181224\\output\\" + "LogBeCompPatch.txt";
            ...
            string err = "";
            if (IsAllPathValid(parInfo, out err))
            {
                //// 文件的头比较
                if (!ByCpFileHeadInfo(bw, parInfo["FilePathA"], parInfo["FilePathB"]))
                {
                    Console.WriteLine("Err : file head info not all same.");
                    bw.Write("Err : file head info not all same.\n".ToArray());
                    //re = false;
                    //return re;
                }
                else
        ...
        /// 校验 init.cfg 
        /// </summary>
        public void BeyondCompareInitCfgMain(string[] args)
        {
            ...
            //string dataBasePath = "D:\\Git_pro\\SCMT\\Src\\SCMT\\Control\\CfgFileOperation\\CfgFileOperation\\bin\\Debug\\";
            string NewFilePath = "";//"D:\\Git_pro\\mib_parser_5g\\src\\net\\init.cfg";
            string YSFilePath = "";//dataBasePath + "5GCfg\\init_qyx.cfg";
            if (parInfo.ContainsKey("FilePathA")==true && parInfo.ContainsKey("FilePathB") == true)
            {
                YSFilePath = parInfo["FilePathA"];
                NewFilePath = parInfo["FilePathB"];
                string err = "";
                if (IsAllPathValid(parInfo, out err))
                {
                    if (BeyondComparedM(bw, YSFilePath, NewFilePath, parInfo))
                    {
            ...
            {
                bw.Write(String.Format("Compare no have file path").ToArray());
            }
        ...
        bool IsAllPathValid(Dictionary<string, string> paths, out string err)
        {
            ...
            if (!File.Exists(paths["FilePathA"]))
            {
                err = "FilePathA: " + paths["DataMdb"] + " 文件不存在！";
                return false;
            }
            if (!File.Exists(paths["FilePathB"]))
            {
                err = "FilePathB: " + paths["Antenna"] + " 文件不存在！";
                return false;
            }
            return true;
        }
        bool BeyondComparedM(BinaryWriter bw, string YSFilePath, string NewFilePath, Dictionary<string, string> parInfo)
        {
            bool re = true;
            ...
            // 比较 每个表的内容
            if (!TestBeyondComFileTableInfoMain(bw, YSFilePath, NewFilePath, parInfo))
            {
                Console.WriteLine("tables info not all same.");
                bw.Write("tables info not all same.\n");
                re = false;
        ...
        /// <returns></returns>
        public bool IsSameStruCfgFileHeader(BinaryWriter bw, StruCfgFileHeader HeadA, StruCfgFileHeader HeadB)
        {
            bool re = true;
            // 只比较意义的字段
            string bug = "";
        ...
        /// <param name="NewFilePath"></param>
        /// <returns></returns>
        bool TestBeyondComFileTableInfoMain(BinaryWriter bw, string YSFilePath, string NewFilePath, Dictionary<string, string> parInfo)
        {
            bool re = true;
            // 获取每个表的偏移位置
            ...
            foreach (string table in YsTableNamePosDict.Keys)
            {
                ...
                if (!TestIsSameLeafHeadInfoList( bw, YsLeafHeadL, NewLeafHeadL))
                {
                ...
                if (!TestIsSameByIndexMain(bw, table, (int)ysTblInfo.u32RecNum, YsLeafHeadL, YsInsts, NewInsts, parInfo))
                {
        ...
        bool TestGetTableInfo(BinaryWriter bw, string YSFilePath, string NewFilePath, Dictionary<string, string> parInfo)
        {
            bool re = true;
            // 获取每个表的偏移位置
            //Dictionary<string, uint> YsTableNamePosDict = GetTableNamesDictByTablesPos(YSFilePath);
            Dictionary<string, uint> NewTableNamePosDict = GetTableNamesDictByTablesPos(NewFilePath);
            // 根据文件头，获取每个表的内容，进行比较
            foreach (string table in NewTableNamePosDict.Keys)
            {
                re = true;
                if (!String.Equals(table, "nrPdcpCfgEntry"))
                {
                    //bw.Write("antennaBfScanWeightEntry continue.\n");
                    //Console.WriteLine("antennaBfScanWeightEntry continue");
                    continue;
        ...
        /// <returns></returns>
        bool TestIsSameLeafHeadInfoList(BinaryWriter bw, Dictionary<string, StruCfgFileFieldInfo> leafHLA, Dictionary<string, StruCfgFileFieldInfo> leafHLB)
        {
            bool re = true;
            //string strBug = "";
            foreach (string leafName in leafHLA.Keys)
            {
                ...
                if (!TestIsSameLeafHeadInfo(bw, leafName, leafHA, leafHB))
                {
                    re = false;
                    TestPrintLeafHeadInfo(bw, leafName, leafHA, leafHB);
                    //strBug = (String.Format("leafname={0}, leafHeadInfo not all same.\n", leafName));
                    //Console.WriteLine(strBug);
                    //bw.Write(strBug.ToArray());
                    //break;
                }
        ...
        /// <returns></returns>
        bool TestIsSameLeafHeadInfo(BinaryWriter bw, string leafName, StruCfgFileFieldInfo leafHA, StruCfgFileFieldInfo leafHB)
        {
            bool re = true;
            string strBug = "";
            /* [48] 字段名 */
            ...
            {
                re = false;
                strBug += (String.Format("FieldName={0}; leafName(a,{1},b.{2}) no same.\n", leafName, strFieldNameA, strFieldNameB));
            }
            if (leafHA.u16FieldOffset != leafHB.u16FieldOffset)/* 字段相对记录头偏移量*/
            {
                re = false;
                strBug += (String.Format("FieldName={0}; u16FieldOffset(a,{1},b.{2}) no same.\n", leafName, leafHA.u16FieldOffset, leafHB.u16FieldOffset));
            }
            if (leafHA.u16FieldLen != leafHB.u16FieldLen)/* 字段长度 单位：字节 */
            {
                re = false;
                strBug += (String.Format("FieldName={0}; u16FieldLen(a,{1},b.{2}) no same.\n", leafName, leafHA.u16FieldLen, leafHB.u16FieldLen));
            }
            if (leafHA.u8FieldType != leafHB.u8FieldType)/* 字段类型 */
            {
                re = false;
                strBug += (String.Format("FieldName={0}; u8FieldType(a,{1},b.{2}) no same.\n", leafName, leafHA.u8FieldType, leafHB.u8FieldType));
            }
            if (leafHA.u8FieldTag != leafHB.u8FieldTag)/* 字段是否为关键字 */
            {
                re = false;
                strBug += (String.Format("FieldName={0}; u8FieldTag(a,{1},b.{2}) no same.\n", leafName, leafHA.u8FieldTag, leafHB.u8FieldTag));
            }
            if (leafHA.u8SaveTag != leafHB.u8SaveTag)/* 字段是否需要存盘 */
            {
                re = false;
                strBug += (String.Format("FieldName={0}; u8SaveTag(a,{1},b.{2}) no same.\n", leafName, leafHA.u8SaveTag, leafHB.u8SaveTag));
            }
            if (leafHA.Getu8ConfigFlag() != leafHB.Getu8ConfigFlag())/* 字段是否可(需要)配置,0:不可配，1：可配*/
            {
                re = false;
                strBug += (String.Format("FieldName={0}; u8ConfigFlag(a,{1},b.{2}) no same.\n", leafName, leafHA.Getu8ConfigFlag(), leafHB.Getu8ConfigFlag()));
            }
            Console.WriteLine(strBug);
            bw.Write(strBug.ToArray());
        ...
        void TestPrintLeafHeadInfo(BinaryWriter bw, string leafName, StruCfgFileFieldInfo leafHA, StruCfgFileFieldInfo leafHB)
        {
            bool re = true;
            string strBug = (String.Format("FieldName={0}; ====info not all some.====.\n", leafName));
            /* [48] 字段名 */
            string strFieldNameA = Encoding.GetEncoding("GB2312").GetString(leafHA.Getu8FieldName()).TrimEnd('\0');
            string strFieldNameB = Encoding.GetEncoding("GB2312").GetString(leafHB.Getu8FieldName()).TrimEnd('\0');
            //if (-1 == String.Compare(strFieldNameA, strFieldNameB))
            {
                re = false;
                strBug += (String.Format("FieldName={0}; leafName(a,{1},b.{2}).\n", leafName, strFieldNameA, strFieldNameB));
            }
            //if (leafHA.u16FieldOffset != leafHB.u16FieldOffset)/* 字段相对记录头偏移量*/
            {
                re = false;
                strBug += (String.Format("FieldName={0}; u16FieldOffset(a,{1},b.{2}).\n", leafName, leafHA.u16FieldOffset, leafHB.u16FieldOffset));
            }
            //if (leafHA.u16FieldLen != leafHB.u16FieldLen)/* 字段长度 单位：字节 */
            {
                re = false;
                strBug += (String.Format("FieldName={0}; u16FieldLen(a,{1},b.{2}).\n", leafName, leafHA.u16FieldLen, leafHB.u16FieldLen));
            }
            //if (leafHA.u8FieldType != leafHB.u8FieldType)/* 字段类型 */
            {
                re = false;
                strBug += (String.Format("FieldName={0}; u8FieldType(a,{1},b.{2}).\n", leafName, leafHA.u8FieldType, leafHB.u8FieldType));
            }
            //if (leafHA.u8FieldTag != leafHB.u8FieldTag)/* 字段是否为关键字 */
            {
                re = false;
                strBug += (String.Format("FieldName={0}; u8FieldTag(a,{1},b.{2}).\n", leafName, leafHA.u8FieldTag, leafHB.u8FieldTag));
            }
            //if (leafHA.u8SaveTag != leafHB.u8SaveTag)/* 字段是否需要存盘 */
            {
                re = false;
                strBug += (String.Format("FieldName={0}; u8SaveTag(a,{1},b.{2}).\n", leafName, leafHA.u8SaveTag, leafHB.u8SaveTag));
            }
            //if (leafHA.u8ConfigFlag != leafHB.u8ConfigFlag)/* 字段是否可(需要)配置,0:不可配，1：可配*/
            {
                re = false;
                strBug += (String.Format("FieldName={0}; u8ConfigFlag(a,{1},b.{2}).\n", leafName, leafHA.Getu8ConfigFlag(), leafHB.Getu8ConfigFlag()));
            }
            Console.WriteLine(strBug);
            bw.Write(strBug.ToArray());
            //Console.WriteLine(String.Format("FieldName={0}; u16FieldOffset(a,{1},b.{2}) ", leafName, leafHA.u16FieldOffset, leafHB.u16FieldOffset));
            ...
            List<byte[]> InstsAList, List<byte[]> InstsBList, Dictionary<string, string> parInfo)
        {
            ...
            List<StruCfgFileFieldInfo> indexLeafsInfo = TestGetIndexLeafInfo(tableName, LeafHead, parInfo);
        ...
        List<StruCfgFileFieldInfo> TestGetIndexLeafInfo(string table, Dictionary<string, StruCfgFileFieldInfo> LeafHead, Dictionary<string, string> parInfo)
        {
            ...
            string dataMdbPath = parInfo["DataMdb"];

            // 表内容
            string strSQL = String.Format(
                "select * from MibTree where MIBName='{0}' and DefaultValue='/' and ICFWriteAble = '√' order by ExcelLine", table);
            DataSet TableMibdateSet = new CfgAccessDBManager().GetRecord(dataMdbPath, strSQL);
            ...
            DataSet MibLeafsDateSet = new CfgAccessDBManager().GetRecord(dataMdbPath, strSQL2);
        ...
        void TestIsSameInstDebugWrite(string strIndex, BinaryWriter bw, string tableName, Dictionary<string, StruCfgFileFieldInfo> LeafHead, byte[] instA, byte[] instB)
        {
            ...
            foreach (string leafName in LeafHead.Keys)
            {
                ...
                if (!BytesCompare_Base64(InA, InB))
                {
                    bw.Write(String.Format("({0}),index({2}),({1}),info:a.(0x{3}),b.(0x{4}) error bugWrire.\n", tableName, leafName, strIndex, inAstr, inBstr).ToArray());
                    Console.WriteLine(String.Format("table({0}),index({2}),leafName({1}),info:a.({3}),b.({4}).", tableName, leafName, strIndex, inAstr, inBstr));//, Encoding.ASCII.GetString(InA), Encoding.Default.GetString(InB)));
                }
                else
                {
                    bw.Write(String.Format("({0}),index({2}),({1}),info:a.(0x{3}),b.(0x{4}) right bugWrire.\n", tableName, leafName, strIndex, inAstr, inBstr).ToArray());
                    Console.WriteLine(String.Format("table({0}),index({2}),leafName({1}),info:a.({3}),b.({4}).", tableName, leafName, strIndex, inAstr, inBstr));//, Encoding.ASCII.GetString(InA), Encoding.Default.GetString(InB)));
                }
            }
            bw.Write(String.Format("\n").ToArray());
