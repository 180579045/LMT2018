...
namespace SCMTOperationCore.Message.SNMP
{
	...
	public sealed class CDTCmdExecuteMgr
	{
			...
			, bool needCheck = false, long timeOut = 0)
		{
			...
			// 根据ip获取当前基站的snmp实例
			ILmtbSnmp lmtbSnmpEx = DTLinkPathMgr.GetInstance().getSnmpByIp(strIpAddr);
							  ...
							  , bool needCheck = false)
		{
			...
			string logMsg = string.Format("cmdName={0},requestId={1}, strIpAddr={2}"
				, cmdName, requestId, strIpAddr);
			Log.Info(logMsg);
