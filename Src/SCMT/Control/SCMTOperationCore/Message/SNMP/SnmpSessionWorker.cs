...
namespace SCMTOperationCore.Message.SNMP
{
	public class SnmpSessionWorker
	{
		...
		{
			packetVb.set_Oid(oid);
			packetVb.set_Value(value);
			packetVb.set_Syntax(syntaxType);
		}
		...
		{
			packetVb.SetAsnType(asnType);

			packetVb.set_Oid(oid);
			packetVb.set_Value(value);
			packetVb.set_Syntax(LmtbSnmpEx.GetSyntax(asnType));
		}
		...
		public static bool SnmpGetSync(string strIpAddr, List<CDTLmtbVb> vbs, out Dictionary<string, string> queryResults, long timeout)
		{
			...
			ILmtbSnmp lmtbSnmp = DTLinkPathMgr.GetInstance().getSnmpByIp(strIpAddr);
			if (lmtbSnmp == null)
			{
