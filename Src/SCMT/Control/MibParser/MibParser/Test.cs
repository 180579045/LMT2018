...
namespace MIBDataParser
{
	class NodeB {
		...
		void testForInitByConnetIp(string connectIp)
		{
			...
			var reCmdData = new Dictionary<string, CmdMibInfo>()
			{
				{ "GetEfdAlarmRule", null},
			};
			if (!dataHandle.GetCmdDataByNames(reCmdData, connectIp, out err))
			{
				Console.WriteLine(err);
			}

			// test_2
			var nameInfo = new Dictionary<string, MibLeaf>() {
			...
			if (null != nameInfo)
				Console.WriteLine("output, {0}", nameInfo["alarmCausePrimaryAlarmCauseNo"].childOid);

			var nameEnSigle = new MibLeaf();
			...
			var tableData =
				new Dictionary<string, MibTable>() {
					{ "alarmCauseTable", null},
				};
			if (!dataHandle.GetMibDataByTableName(tableData, connectIp, out err))
			{
				Console.WriteLine(err);
			}
			else
			...
			// test_5 : 增加查找标量表节点的校验
			MibLeaf reDataTest5 = null;
			if (!dataHandle.GetMibDataByOid("1.3.6.1.4.1.5105.100.1.5.2.1.2.4.0", out reDataTest5, connectIp, out err))
			{
				Console.WriteLine(err);
			}
			else
			{
				Console.WriteLine(String.Format("**Test_5 : In_oid={0}, Out_name_en={1}", "1.3.6.1.4.1.5105.100.1.5.2.1.2.4.0", reDataTest5.childNameMib));
			}
