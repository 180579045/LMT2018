...
namespace MIBDataParser.JSONDataMgr
{
    ...
    class CfgFile_Header
    {
        ...
        public byte[] u8VerifyStr = new byte[4];         // [4]文件头的校验字段 "ICF"  

        public byte u8HiDeviceType;                      //
        public byte u8MiDeviceType;                      //
        public ushort u16LoDevType;                      //
        ...
        public byte[] u8LastMotifyDate = new byte[20];   //  [20]文件最新修改的日期, 按字符串存放 

        public uint u32IcFile_HeaderVer;                 //  初配文件头版本，用于记录不同的文件头格式、版本 
        public uint u32PublicMibVer;                     //  公共Mib版本号
        public uint u32MainMibVer;                       //  Mib主版本号
        public uint u32SubMibVer;                        //  Mib辅助版本号
        public uint u32IcFile_HeaderLength;              //  初配文件头部长度 

        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 256)]
        public byte[] u8IcFileDesc = new byte[256];      //   [256]文件描述 
        ...
        public byte[] u8Pading = new byte[2];            //  [2]
        ...
        public uint[] u32TblOffset = new uint[150];      // [150] 每个表的数据在文件中的起始位置（相对文件头）  

        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 4)]
        public byte[] reserved = new byte[4];            // [4] 保留字段 
    }
    /// <summary>
    /// 为cfg 提供的相关接口实现
    /// </summary>
