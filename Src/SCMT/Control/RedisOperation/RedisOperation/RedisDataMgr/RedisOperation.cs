...
{
    class RedisOperation: IRedisOperation
    {
        static private RedisClient redisClient = null;
        static RedisOperation(){
            RedisClient();
        }
        static void RedisClient()
        {
            try
            {
                redisClient = new RedisClient("127.0.0.1", Int32.Parse("6379"));
                // 测试是否连接
        ...
        /// 实现 接口内容
        public bool RedisClient(string host, string port, out string err)
        {
            err = "";
        ...
        public bool RedisSaveData(IDataRedis data, out string err)
        {
            err = "";
            try {
                redisClient.Set<IDataRedis>(data.KeyTime, data);
            }
            catch (Exception ex)
            {
                err = ex.Message;
        ...
        bool RedisSaveAllData(IDictionary<string, IDataRedis> values, out string err)
        {
            err = "";
            try
            {
                redisClient.SetAll<IDataRedis>(values);
            }
            catch (Exception ex)
            {
                err = ex.Message;
        ...
        /// 
        /// 查找开始/结束时间内的
        bool RedisGetDataByDate(string startYear, string endYear,
            ...
            string startSecond, string endSecond, string startMill, string endMill, out List<string> keyList)
        {
            ...
            this.findPatternList(listStart, listEnd, 0, out PatternMins);
            if (PatternMins.Count > 2)
            {
                ...
                this.findKeyListRightNo2(findKeyList, listStart, listEnd, out keyList);
                //1~end-1
        ...
        bool findPatternList(string[] listStart, string[] listEnd, int effecNum, out List<string> pattList)
        {
            ...
            effecNum = findDataDiffNum(listStart, listEnd) + 1;
            ...
            for (int i = fristDiffStart; i <= fristDiffEnd; i++)
            {
                ...
                string PatternMins = this.effectivePattern2(newlistStart.ToArray(), effecNum + 1);
        ...
        /// 查找 开始/结束时间 不同的时间
        /// </summary>
        int findDataDiffNum(string[] DataStartList, string[] DataEndList)
        {
            int diffNum = -1;
            for (int i = 0; i < 7; i++)
                if (!String.Equals(DataStartList[i], DataEndList[i]))
                    break;
                else
                    diffNum += 1;
            return diffNum;
        ...
        string effectivePattern2(string[] list, int effecNum)
        {
            string effecPat = "*";

            effecPat = DigitsListToString(list, effecNum);
            effecPat += "*";
            return effecPat;
        }
        ...
        bool findKeyListRightNo2(List<string> keyList, string[] listStart, string[] listEnd, out List<string> newKeyList)
        {
        ...
        string DigitsListToString(string[] digitsList, int diffNum)
        {
            int[] listDigit = { 4, 2, 2, 2, 2, 2, 4 };
            string strs = "";
