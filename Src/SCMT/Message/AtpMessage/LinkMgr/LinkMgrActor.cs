...
namespace AtpMessage.LinkMgr
{
    ...
    public class LinkMgrActor
    {
        ...
        public bool ConnectNetElement(string ip, NetElementConfig neConfig)
        {
            ...
            _subscribeClient.AddSubscribeTopic($"from:{ip}:{CommonPort.AtpLinkPort}", OnLinkMsgFromBoard);       //订阅消息
            link.Connect(neConfig);

            return true;
            //TODO 启动定时器，超时后启动取消还是让前台手动取消？
            //TODO 或者设定时间间隔，一直重试？
        }
        ...
        {
            //TODO 多层topic解析，当前版本为了简单，先使用直接调用的方式进行处理
        }
